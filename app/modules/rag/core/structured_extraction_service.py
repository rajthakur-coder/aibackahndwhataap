...
from app.models.rag import FAQ, Policy, ScrapedData, Service, StructuredProduct
from app.modules.ai.core.intelligence_service import detect_policy_type
...
def _replace_existing(db: Session, source_url: str) -> None:
    for model in (StructuredProduct, FAQ, Policy, Service):
        db.execute(delete(model).where(model.source_url == source_url))
