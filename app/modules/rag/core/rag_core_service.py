...
from app.modules.ecommerce.core.ecommerce_core_service import product_knowledge_text
...
from app.modules.rag.core.pinecone_service import query_context, query_matches, upsert_chunks
...
def save_scraped_chunks(db: Session, scraped_data: ScrapedData) -> dict:
    db.execute(
        delete(ScrapedChunk).where(ScrapedChunk.scraped_data_id == scraped_data.id)
    )
...
def find_relevant_website_images(db: Session, query: str, limit: int = 3) -> list[dict]:
    ...
    rows = db.execute(
        select(ScrapedData).order_by(ScrapedData.created_at.desc()).limit(100)
    ).scalars().all()
    scored = []
    for row in rows:
...
def retrieve_relevant_context(db: Session, query: str) -> str:
    ...
    knowledge_chunks = db.execute(
        select(KnowledgeChunk)
        .order_by(KnowledgeChunk.created_at.desc())
        .limit(400)
    ).scalars().all()
    chunks = db.execute(
        select(ScrapedChunk).order_by(ScrapedChunk.created_at.desc()).limit(400)
    ).scalars().all()
    products = db.execute(
        select(EcommerceProduct).order_by(EcommerceProduct.updated_at.desc()).limit(400)
    ).scalars().all()
    ...
    if not chunks:
        chunks = []
        rows = db.execute(
            select(ScrapedData).order_by(ScrapedData.created_at.desc()).limit(20)
        ).scalars().all()
        for row in rows:
...
def _retrieve_structured_context(db: Session, query: str) -> str:
    ...
    if intent.name == "policy_question":
        policies_statement = select(Policy)
        if intent.policy_type:
            policies_statement = policies_statement.where(Policy.policy_type == intent.policy_type)
        policies = db.execute(
            policies_statement.order_by(Policy.created_at.desc()).limit(30)
        ).scalars().all()
        ...
        products = db.execute(
            select(StructuredProduct).order_by(StructuredProduct.created_at.desc()).limit(50)
        ).scalars().all()
        services = db.execute(
            select(Service).order_by(Service.created_at.desc()).limit(50)
        ).scalars().all()
        ...
        faqs = db.execute(
            select(FAQ).order_by(FAQ.created_at.desc()).limit(50)
        ).scalars().all()
