...
from app.modules.whatsapp.webhooks.responses.response_service import (
    ...
    main_menu_buttons as _main_menu_buttons,
...
async def _handle_main_menu(context: WebhookProcessingContext) -> bool:
    if context.understanding.intent not in {"greeting", "menu_request"} and not _is_main_menu_request(context.query_text):
        return False
    with context.timing.stage("whatsapp_send"):
        menu_sent = await _try_send_main_menu(context.phone, context.reply_language, context.bot_settings)
    if not menu_sent:
        return False
    save_message(
        context.db,
        ...
        payload={
            "title": "Main menu",
            "body": getattr(context.bot_settings, "welcome_message", None)
            or _localized(context.reply_language, "How can I help you?", "Kaise help kar sakte hain?"),
            "buttons": [
                {"title": button.get("title") or "Option"}
                for button in _main_menu_buttons(context.bot_settings)
            ],
        },
    )
    return True

async def _handle_catalog_page(context: WebhookProcessingContext) -> bool:
    if not _is_catalog_page_request(context.query_text):
        return False
    return await _send_catalog_category_list(context)

async def _handle_catalog_request(context: WebhookProcessingContext) -> bool:
    if not _looks_like_catalog_request(context.query_text):
        return False
    return await _send_catalog_category_list(context)

async def _send_catalog_category_list(context: WebhookProcessingContext) -> bool:
    category_list_sent = await _try_send_catalog_category_list(
        context.db,
        context.phone,
        context.reply_language,
        page=_catalog_page_number(context.query_text),
    ...
    if category_list_sent:
        save_message(
            ...
            payload={
                "title": "Catalog",
                "body": _localized(context.reply_language, "Which category would you like to view?", "Kaunsi category dekhni hai?"),
                "button_text": "Categories",
            },
        )
    return category_list_sent
