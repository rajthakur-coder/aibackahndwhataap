...
def find_top_selling_products(db: Session, limit: int = 2) -> list[dict]:
    orders = db.execute(
        select(EcommerceOrder).order_by(EcommerceOrder.updated_at.desc()).limit(1000)
    ).scalars().all()
    ...
    products = db.execute(select(EcommerceProduct).limit(1000)).scalars().all()
...
def _ecommerce_candidates(db: Session) -> list[dict]:
    rows = db.execute(
        select(EcommerceProduct).order_by(EcommerceProduct.updated_at.desc()).limit(300)
    ).scalars().all()
    candidates = []
    for row in rows:
