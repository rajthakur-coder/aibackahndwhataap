...
async def get_whatsapp_business_status(
    ...
    if not row:
        raise HTTPException(status_code=404, detail="WhatsApp Business integration not found")
    return {"status": "success", "integration": serialize_integration(row)}


async def list_whatsapp_business_connections(db: AsyncSession, tenant_id: str) -> dict:
